...
class TestCarbonCalculator:
    ...
    def test_travel_car_petrol(self):
        assert cc.calculate_co2("travel", {"mode": "car_petrol", "distance": 25}) == 4.8

    def test_travel_bike_is_zero(self):
        assert cc.calculate_co2("travel", {"mode": "bike", "distance": 100}) == 0.0

    def test_electricity(self):
        assert cc.calculate_co2("electricity", {"units": 20}) == 16.4

    def test_food_meat(self):
        assert cc.calculate_co2("food", {"mealType": "meat", "meals": 3}) == 9.9

    def test_waste_composted(self):
        assert cc.calculate_co2("waste", {"wasteType": "composted", "weight": 2}) == 0.1
    ...
    def test_invalid_travel_mode_raises(self):
        with pytest.raises(cc.InvalidActivityError):
            cc.calculate_co2("travel", {"mode": "teleporter", "distance": 10})

    def test_negative_distance_raises(self):
        with pytest.raises(cc.InvalidActivityError):
            cc.calculate_co2("travel", {"mode": "car_petrol", "distance": -5})

    def test_non_numeric_distance_raises(self):
        with pytest.raises(cc.InvalidActivityError):
            cc.calculate_co2("travel", {"mode": "car_petrol", "distance": "abc"})
...
class TestRecommendation:
    ...
    def test_high_car_km_triggers_carpool_tip(self):
        activities = [{"category": "travel", "mode": "car_petrol", "distance": 30, "co2": 5.76}]
        tips = rec.generate_tips(activities)
        assert any("carpooling" in t for t in tips)

    def test_meat_heavy_diet_triggers_plant_based_tip(self):
        activities = [
            {"category": "food", "meal_type": "meat", "co2": 3.3},
            {"category": "food", "meal_type": "meat", "co2": 3.3},
            {"category": "food", "meal_type": "meat", "co2": 3.3},
        ]
        tips = rec.generate_tips(activities)
        assert any("plant-based" in t for t in tips)

    def test_low_activity_does_not_overtrigger(self):
        activities = [{"category": "travel", "mode": "bike", "distance": 2, "co2": 0.0}]
        tips = rec.generate_tips(activities)
        assert len(tips) >= 1  # falls back to default tip, no crash

    def test_returns_at_most_four_tips(self):
        activities = [
            {"category": "travel", "mode": "car_petrol", "distance": 50, "co2": 9.6},
            {"category": "electricity", "co2": 20},
            {"category": "food", "meal_type": "meat", "co2": 3.3},
            {"category": "food", "meal_type": "meat", "co2": 3.3},
            {"category": "food", "meal_type": "meat", "co2": 3.3},
            {"category": "waste", "waste_type": "landfill", "co2": 6},
        ]
        tips = rec.generate_tips(activities)
        assert len(tips) <= 4
...
def client(tmp_path, monkeypatch):
    """Spin up the Flask app against a temporary, disposable SQLite file
    so tests never touch the real project database."""
    import database
    test_db_dir = tmp_path / "database"
    test_db_dir.mkdir()
    monkeypatch.setattr(database, "DB_DIR", str(test_db_dir))
    monkeypatch.setattr(database, "DB_PATH", str(test_db_dir / "database.db"))
...
class TestAPI:
    ...
    def test_login_creates_user(self, client):
        res = client.post("/api/login", json={"name": "Aditi Rao", "email": "aditi@example.com"})
        assert res.status_code == 200
        body = res.get_json()
    ...
    def test_login_is_idempotent_by_email(self, client):
        r1 = client.post("/api/login", json={"name": "Aditi Rao", "email": "aditi@example.com"})
        r2 = client.post("/api/login", json={"name": "Aditi Rao", "email": "aditi@example.com"})
        assert r1.get_json()["id"] == r2.get_json()["id"]

    def test_login_missing_fields_returns_400(self, client):
        res = client.post("/api/login", json={"name": ""})
        assert res.status_code == 400

    def test_add_activity_valid(self, client):
        res = client.post("/api/activity", json={
            "user_id": 1, "category": "travel", "mode": "car_petrol",
            "distance": 25, "date": "2026-08-01",
        })
        assert res.status_code == 201
        assert res.get_json()["co2"] == 4.8

    def test_add_activity_invalid_returns_400(self, client):
        res = client.post("/api/activity", json={
            "user_id": 1, "category": "travel", "mode": "teleporter", "distance": 10,
        })
        assert res.status_code == 400
        assert "error" in res.get_json()

    def test_dashboard_totals(self, client):
        client.post("/api/activity", json={"user_id": 1, "category": "electricity", "units": 20, "date": "2026-08-01"})
        client.post("/api/activity", json={"user_id": 1, "category": "food", "mealType": "meat", "meals": 3, "date": "2026-08-02"})
        res = client.get("/api/dashboard/1")
        data = res.get_json()
    ...
    def test_goal_set_and_get(self, client):
        client.post("/api/goal", json={"user_id": 1, "target": 100})
        res = client.get("/api/goal/1")
        assert res.get_json()["target"] == 100.0

    def test_goal_rejects_non_positive(self, client):
        res = client.post("/api/goal", json={"user_id": 1, "target": -5})
        assert res.status_code == 400
